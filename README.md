...
| VCC       | 3.3V          | |
| GND       | GND           | |
| RST       | 3.3V（直焊）  | ⚠️ 未接复位电路，见下方「已知问题」 |

> **注意**: P02/P16 是 WS51F6240 的硬件 IIC 引脚，同时也是 I2C 调试下载接口。默认上电有 64ms 延时用于下载握手，如果屏蔽了该延时，需在程序开头自行加延时，否则下次无法下载。
...
**待验证方案**: 将 RST 接到一个 GPIO 或添加 RC 延时电路（如 10kΩ + 1μF），上电后先拉低再拉高。
